...
// OR BREAKOUT BOARD USAGE.  SEE RELEVANT COMMENTS IN Elegoo_TFTLCD.h

// Graphics library by ladyada/Elegoo with init code from Rossum
// MIT license
...
Elegoo_TFTLCD::Elegoo_TFTLCD(void) : Elegoo_GFX(TFTWIDTH, TFTHEIGHT) {
  init();
}

// Initialization common to both shield & breakout configs
void Elegoo_TFTLCD::init(void) {
...
void Elegoo_TFTLCD::begin(uint16_t id) {
  uint8_t i = 0;
...
void Elegoo_TFTLCD::setAddrWindow(int x1, int y1, int x2, int y2) {
  CS_ACTIVE;
  if(driver == ID_932X) {
...
void Elegoo_TFTLCD::flood(uint16_t color, uint32_t len) {
  uint16_t blocks;
...
void Elegoo_TFTLCD::drawFastHLine(int16_t x, int16_t y, int16_t length,
  uint16_t color)
{
  int16_t x2;
...
void Elegoo_TFTLCD::drawFastVLine(int16_t x, int16_t y, int16_t length,
  uint16_t color)
{
  int16_t y2;
...
void Elegoo_TFTLCD::fillRect(int16_t x1, int16_t y1, int16_t w, int16_t h, 
  uint16_t fillcolor) {
  int16_t  x2, y2;
...
void Elegoo_TFTLCD::fillScreen(uint16_t color) {
...
void Elegoo_TFTLCD::drawPixel(int16_t x, int16_t y, uint16_t color) {
...
void Elegoo_TFTLCD::pushColors(uint16_t *data, uint8_t len, boolean first) {
  uint16_t color;
  uint8_t  hi, lo;
  CS_ACTIVE;
...
void Elegoo_TFTLCD::setRotation(uint8_t x) {

  // Call parent rotation func first -- sets up rotation flags, etc.
  Elegoo_GFX::setRotation(x);
...
uint16_t Elegoo_TFTLCD::readPixel(int16_t x, int16_t y) {
...
uint16_t Elegoo_TFTLCD::readID(void) {
...
uint32_t Elegoo_TFTLCD::readReg(uint8_t r) {
  uint32_t id;
  uint8_t x;
...
uint16_t Elegoo_TFTLCD::color565(uint8_t r, uint8_t g, uint8_t b) {
...
#ifndef write8
void Elegoo_TFTLCD::write8(uint8_t value) {
  write8inline(value);
}
#endif

#ifdef read8isFunctionalized
uint8_t Elegoo_TFTLCD::read8fn(void) {
  uint8_t result;
  read8inline(result);
  return result;
}
...
#ifndef setWriteDir
void Elegoo_TFTLCD::setWriteDir(void) {
  setWriteDirInline();
}
#endif

#ifndef setReadDir
void Elegoo_TFTLCD::setReadDir(void) {
  setReadDirInline();
}
#endif

#ifndef writeRegister8
void Elegoo_TFTLCD::writeRegister8(uint8_t a, uint8_t d) {
  writeRegister8inline(a, d);
}
#endif

#ifndef writeRegister16
void Elegoo_TFTLCD::writeRegister16(uint16_t a, uint16_t d) {
  writeRegister16inline(a, d);
}
#endif

#ifndef writeRegisterPair
void Elegoo_TFTLCD::writeRegisterPair(uint8_t aH, uint8_t aL, uint16_t d) {
  writeRegisterPairInline(aH, aL, d);
}
#endif


void Elegoo_TFTLCD::writeRegister24(uint8_t r, uint32_t d) {
  CS_ACTIVE;
  CD_COMMAND;
  write8(r);
  CD_DATA;
...
void Elegoo_TFTLCD::writeRegister32(uint8_t r, uint32_t d) {
  CS_ACTIVE;
  CD_COMMAND;
  write8(r);
  CD_DATA;
